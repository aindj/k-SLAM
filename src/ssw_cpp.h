...
class Alignment {
 public:
  ...
  Alignment() = default;
  Alignment(const Alignment& i)
      : ref_begin(i.ref_begin),
        ref_end(i.ref_end),
        query_begin(i.query_begin),
        query_end(i.query_end),
        sw_score(i.sw_score),
        cigarLen(i.cigarLen) {
    if (i.cigar) {
      cigar = (uint32_t*)malloc(sizeof(uint32_t) * cigarLen);
      std::copy(i.cigar, i.cigar + cigarLen, cigar);
    }
  }
  ...
  Alignment& operator=(const Alignment& i) {
    ref_begin = i.ref_begin;
    ref_end = i.ref_end;
    query_begin = i.query_begin;
    query_end = i.query_end;
  ...
  Alignment& operator=(Alignment&& i) {
    ref_begin = i.ref_begin;
    ref_end = i.ref_end;
    query_begin = i.query_begin;
    query_end = i.query_end;
...
class Aligner {
 public:
  ...
  // =========
  bool Align(const char* query, const Filter& filter,
