...
void BuildSwScoreMatrix(const uint8_t& match_score,
                        const uint8_t& mismatch_penalty, int8_t* matrix) {
  ...
  for (int i = 0; i < 4; ++i) {
    for (int j = 0; j < 4; ++j) {
      matrix[id] =
          ((i == j) ? match_score : static_cast<int8_t>(-mismatch_penalty));
      ++id;
    }
    matrix[id] = 0;
    ++id;
...
void ConvertAlignment(const s_align& s_al, const int& query_len,
  ...
  al->cigar = s_al.cigar;
  al->cigarLen = s_al.cigarLen;
}

int CalculateNumberMismatch(const StripedSmithWaterman::Alignment& al,
                            const int8_t* matrix, int8_t const* ref,
                            int8_t const* query) {

  //  todo ref += al.ref_begin;
...
bool Aligner::Align(const char* query, const Filter& filter,
  ...
  s_align* s_al =
      ssw_align(profile, translated_reference_, reference_length_,
                ...
                filter.score_filter, filter.distance_filter, query_len);

  alignment->Clear();
  ConvertAlignment(*s_al, query_len, alignment);
  //  alignment->mismatches = CalculateNumberMismatch(
  //      *alignment, score_matrix_, translated_reference_, translated_query);
...
//todo optimise by removing translatebase
bool Aligner::Align(const char* query, const char* ref, const int& ref_len,
  ...
  SetFlag(filter, &flag);
  s_align* s_al =
      ssw_align(profile, translated_ref, valid_ref_len,
                static_cast<int>(gap_opening_penalty_),
                static_cast<int>(gap_extending_penalty_), flag,
                filter.score_filter, filter.distance_filter, query_len);
  ...
  ConvertAlignment(*s_al, query_len, alignment);
  //  alignment->mismatches = CalculateNumberMismatch(
  //      *alignment, score_matrix_, translated_ref, translated_query);
...
void Aligner::Clear(void) {
  ...
  if (!default_matrix_ && translation_matrix_) delete[] translation_matrix_;
  translation_matrix_ = NULL;
